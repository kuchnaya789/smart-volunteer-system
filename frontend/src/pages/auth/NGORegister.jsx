...
export default function NGORegister() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const name = res.data?.name || organizationName || "NGO";
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="max-w-xl mx-auto bg-blue-50 border border-blue-100 rounded-xl shadow p-6">
        <div className="text-2xl font-extrabold text-gray-900">NGO Register</div>
        <p className="mt-2 text-sm text-gray-700">Create an NGO account to post tasks and run AI allocations.</p>

        {error && <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}

        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          <div>
            <label className="text-sm font-semibold text-gray-900">Organization Name</label>
            <input
          ...
          <div>
            <label className="text-sm font-semibold text-gray-900">Organization Type</label>
            <input
              className="border border-gray-300 rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-blue-500 mt-1"
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Link to="/ngo/login" className="text-sm font-semibold text-blue-700 hover:underline">
